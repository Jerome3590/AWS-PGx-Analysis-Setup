...
### M5.16XLarge EMR Cluster (Sparklyr/PySpark bootstrap with Python3.7) 
...
aws emr create-cluster \
 ...
 --bootstrap-actions '[{"Args":[],"Name":"pgx","Path":"s3://pgx-terraform/emr-config/pgx_emr_python37_bootstrap.sh"}]' \
 --scale-down-behavior "TERMINATE_AT_TASK_COMPLETION" \
 --ebs-root-volume-size "97" \
 --auto-termination-policy '{"IdleTimeout":3600}' \
 --region "us-east-1" \
...
aws emr describe-cluster --cluster-id ${CLUSTER_ID} --profile pgx | tee ${CWD}/cluster-config/cluster_description.json
...
aws emr list-instances --instance-group-type MASTER --cluster-id ${CLUSTER_ID} --profile pgx | tee ${CWD}/cluster-config/emr_node.json

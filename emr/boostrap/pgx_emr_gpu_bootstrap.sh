...
sudo yum install -y bzip2-devel cairo-devel \
     ...
     pango-devel pango libicu-devel \
...
if [ "$main_node" = true ]; then
    ...
    sudo aws s3 sync s3://pgx-terraform/projects/pgx/PGx_Predictive_Model/ /home/hadoop/
	  sudo aws s3 sync s3://vhi-apcd/Codes/ /home/hadoop/apcd_codes
...
sudo python3 -m pip install boto3 ec2-metadata pyarrow pyspark pandas awswrangler fuzzywuzzy